...
describe('reference-faithful managed public pages', () => {
  it('composes the Artist page as biography, statement and numbered practice notes', () => {
    ...
    expect(screen.getByRole('heading', {name: 'Biography & statement'})).toBeVisible()
    expect(screen.getByRole('heading', {name: 'Practice notes'})).toBeVisible()
    expect(screen.getByRole('heading', {name: 'Explore the practice'})).toBeVisible()
  ...
  it('composes the Studio page with a studio note, process and materials sections', () => {
    ...
    expect(screen.getByRole('img', {name: 'Published process detail'})).toBeVisible()
  ...
  it('composes the Collectors page around three real service paths and its inquiry', () => {
    ...
    expect(within(services).getByRole('link', {name: 'View available works'})).toHaveAttribute(
      'href',
      '/available-works',
    )
    expect(within(services).getByRole('link', {name: 'Arrange a private viewing'})).toHaveAttribute(
      'href',
      '/private-viewings',
    )
    expect(within(services).getByRole('link', {name: 'Discuss a commission'})).toHaveAttribute(
      'href',
      '/commission-a-work',
    )
    expect(screen.getByRole('region', {name: 'Collector inquiry'})).toBeVisible()
  ...
  it('composes the Turkish Commission page with a five-step process, FAQ and form', () => {
    ...
    expect(screen.getByRole('heading', {name: 'Özel eser süreci'})).toBeVisible()
    expect(screen.getByRole('list', {name: 'Özel eser süreci'})).toHaveTextContent(
      '01',
    )
    expect(screen.getByRole('list', {name: 'Özel eser süreci'}).children).toHaveLength(5)
    expect(screen.getByRole('heading', {name: 'Sık sorulan sorular'})).toBeVisible()
    expect(screen.getAllByRole('group')).toHaveLength(3)
    expect(screen.getByRole('region', {name: 'Commission inquiry'})).toBeVisible()
  })

  it('composes Private Viewings with benefits, expectations and the real inquiry slot', () => {
    render(
      <PublicPrivateViewingsPage
        inquiry={<section aria-label="Private viewing inquiry">Inquiry</section>}
        locale="en"
        page={page}
      />,
    )

    expect(screen.getByRole('region', {name: 'A closer encounter'})).toBeVisible()
    expect(screen.getByRole('heading', {name: 'What to expect'})).toBeVisible()
    expect(screen.getByRole('heading', {name: 'Request a private viewing'})).toBeVisible()
    expect(screen.getByRole('region', {name: 'Private viewing inquiry'})).toBeVisible()
    expect(screen.queryByText(/monday|saturday|hours|shipping|certificate/iu)).toBeNull()
  })
})
