...
import {workshopItemSchema, workshopSettingsSchema} from '@/schemas/workshop'
import {createServerAction} from '@/utils/create-server-action'
import {createClient, getUser} from '@/utils/supabase/server'
import {isValidUUID} from '@/utils/validation'

// Schema for workshop data save
const workshopSaveSchema = z.object({
  items: z.array(workshopItemSchema),
  settings: workshopSettingsSchema,
})

// Schema for workshop item operations
const workshopItemOperationSchema = z.object({
  id: z.string().min(1, 'Item ID is required'),
})

const workshopItemUpdateSchema = workshopItemSchema.extend({
  id: z.string().min(1, 'Item ID is required'),
})

// Create new workshop item
export const createWorkshopItemAction = createServerAction(
  workshopItemSchema,
  async ({input}) => {
    ...
    return {
      success: true,
      data,
      message: 'Workshop item created successfully',
    }
  },
)

// Delete workshop item
export const deleteWorkshopItemAction = createServerAction(
  workshopItemOperationSchema,
  async ({input}) => {
    const user = await getUser()
...
// Get workshop data
export const getWorkshopDataAction = createServerAction(
  z.object({}),
  async () => {
    const user = await getUser()
    ...
    if (workshopError && workshopError.code !== 'PGRST116') {
      throw new Error(`Database error: ${workshopError.message}`)
    }
...
// Save workshop data (bulk update)
export const saveWorkshopDataAction = createServerAction(
  workshopSaveSchema,
  async ({input}) => {
    try {
      console.log('saveWorkshopDataAction called with input:', input)
      ...
      console.log('Upserting workshop settings...')
      ...
      console.error('saveWorkshopDataAction error:', error)
      throw error
    }
  },
)

// Update single workshop item
export const updateWorkshopItemAction = createServerAction(
  workshopItemUpdateSchema,
  async ({input}) => {
    const user = await getUser()
