...
Bu V2 listesi yalnız kanıtlanmış işler `[x]` olarak işaretlenerek ilerletilir.
V1 production dönüşüm geçmişi referans için aşağıda korunur.

## V2 Faz 0 — Tasarım ve kontrollü temiz başlangıç

- [x] Editorial heritage yönünü ve referans ekranlarını onayla
- [x] Public hesapların kaldırılmasına karar ver
- [x] Satış yerine availability inquiry / commission / private viewing modelini
      onayla
- [x] Harici CMS yerine Bekten Studio yaklaşımını onayla
- [x] V2 mimari, veri, güvenlik, test ve geçiş tasarımını belgele
- [ ] V1 kaldırma envanterini kaynak yoluyla çıkar
- [ ] Yeniden kullanılabilecek V1 parçalarını geçici `backup/` yapısına taşı
- [ ] Public auth, profil, kullanıcı araçları ve satış/store yüzeyini kaldır
- [ ] Eski admin UI'ı kaldır ve yeni Studio için teknik altyapıyı koru
- [ ] Kullanılmayan auth/store/UI bağımlılıklarını ve env sözleşmelerini temizle
- [ ] Temizlik sonrası lint, typecheck, test ve build durumunu kaydet
- [ ] V1 cleanup commit'ini oluştur

## V2 Faz 1 — Domain ve içerik altyapısı

- [ ] `Collection`, `Exhibition`, `JournalEntry`, `Page`, `Inquiry` ve
      `ContentRevision` domainlerini test-first ekle
- [ ] Artwork modelini availability, collection ve editoryal metadata ile geliştir
- [ ] Locale alanları, slug ve draft/published/archived yaşam döngüsünü tamamla
- [ ] Demo seed içeriğini düzenlenebilir ve production-gated biçimde oluştur
- [ ] Revision oluşturma, karşılaştırma ve immutable restore akışını tamamla
- [ ] Publish transaction + cache invalidation akışını tamamla

## V2 Faz 2 — Bekten Studio erişimi ve shell

- [ ] Public NextAuth/login/register/reset/profile yüzeylerini tamamen kaldır
- [ ] Resend magic-link token akışını RED→GREEN tamamla
- [ ] EDITOR/OWNER authorization ve recent-auth sınırlarını tamamla
- [ ] Bekten Studio responsive shell ve sade navigasyonu geliştir
- [ ] Owner-only operasyon araçlarını editörden ayır
- [ ] Studio audit, error state ve erişilebilirlik testlerini tamamla

## V2 Faz 3 — Studio içerik yönetimi

- [ ] Genel bakış ve görev odaklı boş durumları geliştir
- [ ] Eserler ve koleksiyonlar CRUD + sıralama + preview/publish akışını geliştir
- [ ] Sergiler CRUD + preview/publish akışını geliştir
- [ ] Journal, press ve sayfalar CRUD + preview/publish akışını geliştir
- [ ] Garage medya yükleme/seçme/sıralama/alt text deneyimini geliştir
- [ ] Inquiry inbox, filtre, durum, not ve assignment akışını geliştir
- [ ] Revision geçmişi ve geri alma deneyimini geliştir

## V2 Faz 4 — Public editorial heritage deneyimi
...
- [ ] Home, Collections, Works ve Work detail sayfalarını geliştir
- [ ] Available works ve Collectors sayfalarını geliştir
- [ ] Exhibitions ve Exhibition detail sayfalarını geliştir
- [ ] Artist, Studio, Archive, Journal ve Press sayfalarını geliştir
- [ ] Commission ve Private viewing anlatı sayfalarını geliştir
- [ ] Availability/commission/private-viewing formlarını geliştir
- [ ] Demo içeriğin Studio'dan tamamen değiştirilebilir olduğunu doğrula

## V2 Faz 5 — Kalite ve production

- [ ] Inquiry + outbox + Resend bildirimlerini idempotent tamamla
- [ ] Garage private/public medya ve güvenlik sınırlarını doğrula
- [ ] 4 locale çeviri, canonical, hreflang, sitemap ve structured data'yı tamamla
...
- [ ] Coolify production env/migration/deploy/health doğrulamasını tamamla
- [ ] Geçici `backup/` klasörünü tamamen sil
- [ ] Final requirement-by-requirement completion audit yap

---

# Bekten Art V1 production dönüşümü
